{{-- resources/views/staff/shift.blade.php --}}
@extends('layouts.admin')

@section('title', 'Shift Management - Digital Clean Solution')

@section('page-content')
<div x-data="shiftData()" x-init="fetchItems()" class="max-w-5xl mx-auto px-4">

    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:w-72">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" x-model="search" @input.debounce.300ms="fetchItems()" placeholder="Search shifts..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Title</th>
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Start</th>
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">End</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="items.length === 0">
                        <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">No shifts found.</td></tr>
                    </template>
                    <template x-for="(item, index) in items" :key="item.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="(currentPage - 1) * perPage + index + 1"></td>
                            <td class="px-4 py-4 text-sm text-gray-900 font-medium whitespace-nowrap" x-text="item.title"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.startTime || ''"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.endTime || ''"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.totalTime || ''"></td>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-semibold text-gray-800" x-text="isEditing ? 'Edit Shift' : 'Add Shift'"></h3>
            ...
            <form @submit.prevent="saveItem()" class="space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.title" placeholder="Shift title"
                ...
                <div>
                    ...
                    <textarea x-model="form.description" rows="2" placeholder="Shift description..."
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="time" x-model="form.startTime"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">End Time</label>
                        <input type="time" x-model="form.endTime"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Time Cushion</label>
                        <input type="text" x-model="form.timeCushion" placeholder="e.g. 15 min"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Total Time</label>
                    <input type="text" x-model="form.totalTime" placeholder="e.g. 8 hours"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                </div>
    ...
    <div x-show="deleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-2">Delete Shift?</h3>
...
function shiftData() {
    return {
        ...
        isEditing: false,
        saving: false,
        form: { id: null, title: '', description: '', startTime: '', endTime: '', timeCushion: '', totalTime: '' },
        errors: {},
        deleteId: null,
        ...
        async fetchItems() {
            try {
                ...
                let res = await fetch('/api/v1/shift?' + params.toString(), {
        ...
        openAddModal() {
            ...
            this.form = { id: null, title: '', description: '', startTime: '', endTime: '', timeCushion: '', totalTime: '' };
            this.errors = {};
            this.modalOpen = true;
        },
        ...
        async saveItem() {
            this.errors = {};
            if (!this.form.title) { this.errors.title = 'Shift title is required'; return; }
            this.saving = true;
            try {
                let token = localStorage.getItem('S_S_Token');
                let url = '/api/v1/shift';
                let method = 'POST';
        ...
        async deleteItem() {
            ...
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/shift/' + this.deleteId, {
                    method: 'DELETE',
