...
@section('title', 'Customer Management - Digital Clean Solution')

@section('page-content')
<div x-data="customerData()" x-init="fetchCustomers()" class="max-w-7xl mx-auto px-4">
    {{-- Header --}}
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody>
                    <template x-for="(customer, index) in paginatedCustomers" :key="customer.id">
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 text-gray-600" x-text="(currentPage - 1) * pageSize + index + 1"></td>
                            <td class="px-4 py-3 font-medium text-gray-800" x-text="customer.name"></td>
                            <td class="px-4 py-3 text-gray-600" x-text="customer.email"></td>
                            <td class="px-4 py-3 text-gray-600" x-text="customer.phone"></td>
                            <td class="px-4 py-3 text-gray-600" x-text="customer.country"></td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button @click="openEditModal(customer)" class="text-blue-600 hover:text-blue-800 p-1.5 rounded-lg hover:bg-blue-50 transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button @click="openDeleteModal(customer)" class="text-red-600 hover:text-red-800 p-1.5 rounded-lg hover:bg-red-50 transition-colors" title="Delete">
                    ...
                    <tr x-show="paginatedCustomers.length === 0 && !loading">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center px-4 py-3 border-t border-gray-100 gap-3" x-show="totalPages > 1">
            <span class="text-sm text-gray-600" x-text="'Showing ' + ((currentPage - 1) * pageSize + 1) + '-' + Math.min(currentPage * pageSize, filteredCustomers.length) + ' of ' + filteredCustomers.length"></span>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div class="bg-primary text-white px-6 py-4 rounded-t-2xl flex justify-between items-center sticky top-0 z-10">
                <h2 class="text-lg font-semibold" x-text="editId ? 'Edit Customer' : 'Add Customer'"></h2>
            ...
            <form @submit.prevent="saveCustomer()" class="p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="email" x-model="form.email" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" x-model="form.countryCode" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Country *</label>
                        <input type="text" x-model="form.country" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">City</label>
                        <input type="text" x-model="form.city" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Contact Person 1</label>
                        <input type="text" x-model="form.contactPerson1" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Contact Person 2</label>
                        <input type="text" x-model="form.contactPerson2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                </div>
                <div class="col-span-full">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                    <textarea x-model="form.address" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"></textarea>
                </div>
                <div x-show="!editId">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Password *</label>
                    <input type="password" x-model="form.password" :required="!editId" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
    ...
    <div x-show="deleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
            ...
            <p class="text-gray-600 mb-6">Are you sure you want to delete <span class="font-semibold" x-text="deleteTarget?.name"></span>?</p>
            <div class="flex justify-center gap-3">
                ...
                <button @click="deleteCustomer()" :disabled="saving" class="bg-red-600 text-white px-5 py-2.5 rounded-lg hover:bg-red-700 transition-colors shadow-md disabled:opacity-50" x-text="saving ? 'Deleting...' : 'Delete'"></button>
            </div>
...
function customerData() {
    return {
        ...
        currentPage: 1,
        pageSize: 10,
        form: { name: '', email: '', phone: '', countryCode: '', country: '', city: '', address: '', contactPerson1: '', contactPerson2: '', password: '' },
        ...
        async fetchCustomers() {
            ...
            this.errorMsg = '';
            try {
                const res = await fetch('/api/v1/customer', { headers: this.authHeaders });
                const data = await res.json();
                if (data.status) {
                    this.customers = Array.isArray(data.data) ? data.data : [];
                } else {
                    this.errorMsg = data.message || 'Failed to load customers';
                }
            } catch (e) {
        ...
        openAddModal() {
            this.editId = null;
            this.errorMsg = '';
            this.form = { name: '', email: '', phone: '', countryCode: '', country: '', city: '', address: '', contactPerson1: '', contactPerson2: '', password: '' };
            this.modalOpen = true;
        },
        ...
        openEditModal(customer) {
            ...
            this.form = {
                ...
                countryCode: customer.country_code || customer.countryCode || '',
                country: customer.country || '',
                city: customer.city || '',
                address: customer.address || '',
                contactPerson1: customer.contact_person1 || customer.contactPerson1 || '',
                contactPerson2: customer.contact_person2 || customer.contactPerson2 || '',
                password: ''
            };
            this.modalOpen = true;
        },
        ...
        async saveCustomer() {
            ...
            const url = this.editId ? '/api/v1/customer/' + this.editId : '/api/v1/customer';
            const body = { ...this.form };
            if (this.editId) delete body.password;
            try {
                const res = await fetch(url, { method, headers: this.authHeaders, body: JSON.stringify(body) });
                const data = await res.json();
                if (data.status) {
                    this.closeModal();
        ...
        async deleteCustomer() {
            ...
            this.errorMsg = '';
            try {
                const res = await fetch('/api/v1/customer/' + this.deleteTarget.id, { method: 'DELETE', headers: this.authHeaders });
                const data = await res.json();
                if (data.status) {
