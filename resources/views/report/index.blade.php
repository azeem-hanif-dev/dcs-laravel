...
@endsection
@push('scripts')<script>function reportsData(){return{summary:{totalSales:0,outstanding:0,totalPO:0,stockValue:0},topShops:[],outstandingInvoices:[],loading:true,async loadData(){this.loading=true;try{var d=await $store.api.get('/api/v1/dashboard/counts');if(d&&d.status)this.summary=d.data;var so=await $store.api.get('/api/v1/sales-order',{per_page:100,status:'Delivered'});if(so&&so.status){var orders=Array.isArray(so.data)?so.data:(so.data?.data||[]);var shopMap={};orders.forEach(function(o){var sid=o.shop_id;if(!shopMap[sid])shopMap[sid]={id:sid,name:o.shop?.name||'',orderCount:0,totalAmount:0};shopMap[sid].orderCount++;shopMap[sid].totalAmount+=parseFloat(o.grand_total||0)});this.topShops=Object.values(shopMap).sort(function(a,b){return b.totalAmount-a.totalAmount}).slice(0,5)}var inv=await $store.api.get('/api/v1/invoice',{per_page:50,status:'Unpaid'});if(inv&&inv.status)this.outstandingInvoices=(Array.isArray(inv.data)?inv.data:(inv.data?.data||[])).filter(function(i){return(i.balance_due||0)>0}).sort(function(a,b){return new Date(a.due_date)-new Date(b.due_date)})}catch(e){console.error(e)}this.loading=false}}}</script>@endpush
