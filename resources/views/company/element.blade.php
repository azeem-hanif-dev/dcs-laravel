...
@section('title', 'Element Management')
...
<div class="p-6" x-data="{
    ...
    async fetchData() {
        ...
        try {
            ...
            const response = await fetch('/api/v1/element', {
    ...
    async addItem() {
        ...
        try {
            ...
            if (data.status) {
                ...
                this.showNotification(data.message || 'Element added successfully!');
            } else {
    ...
    async updateItem() {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/v1/element/' + this.editItem.id, {
                method: 'PUT',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Content-Type': 'application/json',
            ...
            if (data.status) {
                ...
                this.editName = '';
                await this.fetchData();
                this.showNotification(data.message || 'Element updated successfully!');
            } else {
                throw new Error(data.message || 'Failed to update element');
            }
        } catch (err) {
            this.showNotification(err.message, 'error');
        }
    ...
    async deleteItem() {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/v1/element/' + this.deleteId, {
                method: 'DELETE',
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json'
            ...
            if (data.status) {
                ...
                this.deleteId = null;
                await this.fetchData();
                this.showNotification(data.message || 'Element deleted successfully!');
            } else {
    ...
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-semibold">Element Management</h1>
        ...
        <table class="table-fixed min-w-full bg-white border border-gray-200 text-sm">
            ...
            <tbody>
                <template x-if="paginatedData.length === 0">
                    <tr>
                        <td colspan="3" class="text-center py-6 text-gray-500">No elements found.</td>
                    </tr>
                </template>
    ...
    <div x-show="showAddModal"
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in" @click.outside="showAddModal = false">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Add Element</h2>
    ...
    <div x-show="showEditModal"
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in" @click.outside="showEditModal = false">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Edit Element</h2>
    ...
    <div x-show="showDeleteModal"
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-xl p-6 w-[90%] max-w-sm text-center animate-fade-in" @click.outside="showDeleteModal = false">
            ...
            <p class="text-gray-600 mb-6">Are you sure you want to delete this element? This action cannot be undone.</p>
