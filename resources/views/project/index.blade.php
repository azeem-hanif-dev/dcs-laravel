...
function projectData(){return{
    ...
    async init(){try{this.pager=Alpine.store('pager').create({endpoint:'/api/v1/project',perPage:10})}catch(e){console.error('Pager create failed:',e);this.pager.loading=false;return}this.fetchDependencies();this.pager.fetchPage()},
    doSearch(){try{this.pager.setSearch({search:this.search||undefined})}catch(e){this.search=''}},
    async fetchDependencies(){try{var cr=await Alpine.store('api').get('/api/v1/customer',{per_page:200});var sr=await Alpine.store('api').get('/api/v1/staff',{per_page:200});if(cr&&cr.status)this.customers=Array.isArray(cr.data)?cr.data:(cr.data?.data||[]);if(sr&&sr.status)this.supervisors=Array.isArray(sr.data)?sr.data:(sr.data?.data||[])}catch(e){console.error('Failed to load project dependencies:',e)}},
    ...
    async saveProject(){this.errors={};if(!this.form.name){this.errors.name='Name is required';return}this.saving=true;try{if(this.editId){await Alpine.store('api').put('/api/v1/project/'+this.editId,this.form)}else{await Alpine.store('api').post('/api/v1/project',this.form)}this.closeModal();Alpine.store('toast').success(this.editId?'Project updated':'Project created');this.pager.refresh()}catch(e){if(e.errors)this.errors=e.errors;else Alpine.store('toast').error(e.message||'Save failed')}finally{this.saving=false}},
    async deleteProject(){if(!this.deleteTarget)return;this.saving=true;try{await Alpine.store('api').del('/api/v1/project/'+this.deleteTarget.id);this.deleteModalOpen=false;this.deleteTarget=null;Alpine.store('toast').success('Project deleted');this.pager.refresh()}catch(e){Alpine.store('toast').error(e.message||'Delete failed')}finally{this.saving=false}}
}}
</script>
@endpush
