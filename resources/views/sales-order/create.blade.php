...
function soCreateData(){return{
    ...
    async fetchShops(){try{var d=await $store.api.get('/api/v1/shop',{per_page:500});if(d&&d.status)this.shops=Array.isArray(d.data)?d.data:(d.data?.data||[])}catch(e){console.error(e)}},
    async fetchSalesmen(){try{var d=await $store.api.get('/api/v1/salesman',{per_page:500});if(d&&d.status)this.salesmen=Array.isArray(d.data)?d.data:(d.data?.data||[])}catch(e){console.error(e)}},
    async fetchWarehouses(){try{var d=await $store.api.get('/api/v1/warehouse',{per_page:500});if(d&&d.status)this.warehouses=Array.isArray(d.data)?d.data:(d.data?.data||[])}catch(e){console.error(e)}},
    async fetchProducts(){try{var d=await $store.api.get('/api/v1/material',{per_page:500,status:'Active'});if(d&&d.status)this.products=Array.isArray(d.data)?d.data:(d.data?.data||[])}catch(e){console.error(e)}},
    ...
    async saveOrder(status){this.errors={};if(!this.form.shop_id){this.errors.shop_id='Please select a shop';return}if(this.items.length===0){$store.toast.error('Add at least one product');return}this.calcTotals();this.saving=true;var payload={...this.form,status:status,items:this.items.map(function(i){return{product_id:i.product_id,quantity:i.quantity,unit_price:i.unit_price}})};try{var d=await $store.api.post('/api/v1/sales-order',payload);if(d.status){$store.toast.success(status==='Confirmed'?'Order confirmed!':'Draft saved!');window.location.href='/company_admin/sales_orders'}else{$store.toast.error(d.message||'Failed to save')}}catch(e){$store.toast.error(e.message||'Save failed')}finally{this.saving=false}}
}}
</script>
@endpush
