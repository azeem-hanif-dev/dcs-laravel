{{-- resources/views/method/health.blade.php --}}
@extends('layouts.admin')

@section('title', 'Health & Safety Management - Digital Clean Solution')

@section('page-content')
<div class="max-w-7xl mx-auto px-2" x-data="healthSafetyData()" x-init="init()">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Health & Safety Management</h1>
                <p class="text-sm text-gray-500 mt-1">Manage health and safety guidelines and procedures</p>
            </div>
        ...
        <div class="p-4 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300="fetchData()" placeholder="Search safety methods..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
            </div>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.outside="closeModal()">
            <div class="flex items-center justify-between p-5 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-800" x-text="editingId ? 'Edit Safety Method' : 'Add Safety Method'"></h2>
            ...
            <div class="p-5 space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.title" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none" placeholder="Enter safety method title">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <textarea x-model="form.description" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none resize-none" placeholder="Enter safety method description..."></textarea>
                </div>
                <div>
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm" @click.outside="showDeleteModal = false">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Delete Safety Method</h3>
                <p class="text-sm text-gray-500 mb-6">Are you sure you want to delete this safety method? This action cannot be undone.</p>
    ...
    return {
        ...
        async fetchData() {
            ...
            try {
                ...
                let res = await fetch('/api/v1/safety/method?' + params, { headers: this.getHeaders() });
        ...
        async fetchCategories() {
            try {
                let res = await fetch('/api/v1/safety/method-category?per_page=all', { headers: this.getHeaders() });
                let data = await res.json();
        ...
        async saveItem() {
            ...
            this.saving = true;
            try {
                let url = this.editingId ? '/api/v1/safety/method/' + this.editingId : '/api/v1/safety/method';
                ...
                let data = await res.json();
                if (res.ok) {
                    this.showToast(this.editingId ? 'Safety method updated' : 'Safety method created', 'success');
                    this.closeModal();
                    this.fetchData();
                } else {
        ...
        async deleteItem() {
            this.deleting = true;
            try {
                let res = await fetch('/api/v1/safety/method/' + this.deleteTarget.id, { method: 'DELETE', headers: this.getHeaders() });
                if (res.ok) {
                    this.showToast('Safety method deleted', 'success');
