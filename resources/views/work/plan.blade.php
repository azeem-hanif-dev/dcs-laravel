...
@section('title', 'Work Plan Management - Digital Clean Solution')

@section('page-content')
<div x-data="workPlanData()" x-init="fetchDependencies().then(() => fetchWorkPlans())" class="max-w-7xl mx-auto px-4">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 class="text-2xl font-bold text-gray-800">Work Plan Management</h1>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody>
                    <template x-for="(item, index) in paginatedWorkPlans" :key="item.id">
                    ...
                    <tr x-show="paginatedWorkPlans.length === 0 && !loading">
                        <td colspan="7" class="px-4 py-12 text-center text-gray-500">No work plans found.</td>
                    </tr>
                    <tr x-show="loading">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center px-4 py-3 border-t border-gray-100 gap-3" x-show="totalPages > 1">
            <span class="text-sm text-gray-600" x-text="'Showing ' + ((currentPage - 1) * pageSize + 1) + '-' + Math.min(currentPage * pageSize, filteredWorkPlans.length) + ' of ' + filteredWorkPlans.length"></span>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="bg-primary text-white px-6 py-4 rounded-t-2xl flex justify-between items-center sticky top-0 z-10">
                <h2 class="text-lg font-semibold" x-text="editId ? 'Edit Work Plan' : 'Add Work Plan'"></h2>
...
function workPlanData() {
    return {
        ...
        get filteredWorkPlans() {
            ...
            if (!q) return this.workPlans;
            return this.workPlans.filter(w =>
        ...
        get totalPages() {
            return Math.ceil(this.filteredWorkPlans.length / this.pageSize) || 1;
        },

        get paginatedWorkPlans() {
            const start = (this.currentPage - 1) * this.pageSize;
            return this.filteredWorkPlans.slice(start, start + this.pageSize);
        },
        ...
        openAddModal() {
            ...
            this.errorMsg = '';
            this.jobs = [];
            this.form = { projectId: '', jobId: '', workerId: '', jobType: '', days: [], weeks: '', date: '', status: '' };
            this.modalOpen = true;
        },
        ...
        async saveWorkPlan() {
            ...
            const url = this.editId ? '/api/v1/work/' + this.editId : '/api/v1/work';
            const body = {
                ...this.form,
