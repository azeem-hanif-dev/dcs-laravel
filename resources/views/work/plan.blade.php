...
function workPlanData(){return{
    ...
    async init(){try{this.pager=Alpine.store('pager').create({endpoint:'/api/v1/work',perPage:10})}catch(e){console.error('Pager create failed:',e);this.pager.loading=false;return}this.fetchDependencies();this.pager.fetchPage()},
    doSearch(){try{this.pager.setSearch({search:this.search||undefined})}catch(e){this.search=''}},
    async fetchDependencies(){try{var pd=await Alpine.store('api').get('/api/v1/project',{per_page:200});var sd=await Alpine.store('api').get('/api/v1/staff',{per_page:200});if(pd&&pd.status)this.projects=Array.isArray(pd.data)?pd.data:(pd.data?.data||[]);if(sd&&sd.status)this.workers=Array.isArray(sd.data)?sd.data:(sd.data?.data||[])}catch(e){console.error('Failed to load work plan dependencies:',e)}},
    async fetchJobs(pid){this.jobs=[];if(!pid)return;try{var d=await Alpine.store('api').get('/api/v1/job/project/'+pid);if(d&&d.status)this.jobs=Array.isArray(d.data)?d.data:(d.data?.data||[])}catch(e){console.error('Failed to load jobs:',e)}},
