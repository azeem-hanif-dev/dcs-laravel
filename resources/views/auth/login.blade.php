...
<div class="flex h-screen overflow-hidden bg-[#0b1a2e] relative" x-data="{
    ...
    <div class="relative z-10 w-full flex items-center justify-center px-4">
        <div class="w-full max-w-md">
            {{-- Card --}}
            <div class="bg-[#153359] rounded-2xl shadow-2xl p-8 border border-[#2EA8FA]/10">
                {{-- Logo inside card --}}
                <div class="flex justify-center mb-6">
                    <img src="/common/distributor-logo.svg" alt="Distributor Logo"
                        class="h-20 w-auto logo-white animate-logo-pulse drop-shadow-[0_0_18px_rgba(46,168,250,0.5)]" />
                </div>
                    ...
                    <form @submit.prevent="async () => {
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" x-model="companyName" required
                                class="w-full px-3 bg-[#0f2744] text-white py-3 border border-[#2EA8FA]/30 rounded-md focus:outline-none focus:ring-2 focus:ring-[#2EA8FA] focus:border-transparent transition-all"
                                placeholder="Enter your company name" autofocus />
                        </div>
                        <button type="submit" :disabled="loading"
                            class="w-full text-white bg-primary py-3 px-4 rounded-md hover:bg-primary-dark font-medium transition-colors">
                            <span x-show="!loading">Continue</span>
                            <span x-show="loading">Verifying...</span>
                        </button>
                        <p x-show="errorMsg" class="mt-4 text-center text-red-400 text-sm" x-text="errorMsg"></p>
                    </form>
                </div>
                ...
                <div x-show="step === 2" x-transition>
                    <div class="flex items-center mb-4">
                        <button @click="step = 1; errorMsg = ''" class="text-gray-400 hover:text-white mr-3 p-1 rounded-md hover:bg-[#0f2744] transition-colors">
                    ...
                    <form @submit.prevent="async () => {
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" @click="showPassword = !showPassword" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-200 transition-colors">
                        ...
                        <div class="flex justify-end mb-6">
                            <button type="button" @click="modalOpen = true; forgotCompanyId = companyData?.companyId"
                                class="text-sm text-primary font-sans hover:underline transition-colors">Forget Password?</button>
                        </div>
                        <button type="submit" :disabled="loading"
                            class="w-full text-white bg-primary py-3 px-4 rounded-md hover:bg-primary-dark font-medium transition-colors">
            ...
            <div class="flex mt-8 flex-col items-center space-y-1">
                    ...
                    <p class="text-center text-sm text-gray-400 font-light hover:text-gray-300 transition-colors">Powered By Digital Stationz</p>
                </a>
            </div>
        </div>
    </div>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" x-transition @click.outside="modalOpen = false">
        <div class="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md">
            ...
            <form @submit.prevent="async () => {
                ...
                <input type="email" x-model="forgotEmail" required placeholder="Enter your email" class="w-full px-3 py-2 border rounded-md mb-4" />
                <div class="flex gap-3">
                    <button type="button" @click="modalOpen = false" class="flex-1 px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300 transition-colors">Cancel</button>
                    <button type="submit" :disabled="loading" class="flex-1 px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors">Send Reset Link</button>
                </div>
