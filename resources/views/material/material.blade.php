...
function materialData(){return{
    ...
    async fetchItems(){this.loading=true;try{var d=await $store.api.get('/api/v1/material',{page:this.currentPage,per_page:this.perPage,search:this.search||undefined,categoryId:this.filterCategoryId||undefined,status:this.filterStatus||undefined});if(d.status){this.items=d.data?.data||d.data||[];this.total=d.data?.total||d.total||this.items.length;this.totalPages=d.data?.last_page||d.last_page||Math.ceil(this.total/this.perPage)||1}}catch(e){$store.toast.error('Failed to load materials')}this.loading=false},
