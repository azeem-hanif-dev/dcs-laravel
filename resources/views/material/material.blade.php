...
@section('title', 'Material Management - Digital Clean Solution')

@section('page-content')
<div x-data="materialData()" x-init="init()" class="max-w-7xl mx-auto px-4">

    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:w-72">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" x-model="search" @input.debounce.300ms="fetchItems()" placeholder="Search materials..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Qty</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="items.length === 0">
                        <tr><td colspan="7" class="px-6 py-12 text-center text-gray-500">No materials found.</td></tr>
                    </template>
                    <template x-for="(item, index) in items" :key="item.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="(currentPage - 1) * perPage + index + 1"></td>
                            <td class="px-4 py-4 text-sm text-gray-900 font-medium whitespace-nowrap" x-text="item.materialName"></td>
                            <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap" x-text="item.category?.name || ''"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="'&euro;' + parseFloat(item.price).toFixed(2)"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.totalQuantity"></td>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="saveItem()" class="space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.materialName" placeholder="Enter material name"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select x-model="form.categoryId" @change="fetchSubcategories()"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                        ...
                        <p x-show="errors.categoryId" class="text-red-500 text-xs mt-1" x-text="errors.categoryId"></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Subcategory</label>
                        <select x-model="form.subcategoryId"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" x-model="form.totalQuantity" placeholder="0"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Assigned Quantity</label>
                        <input type="number" x-model="form.assignedQuantity" placeholder="0"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Uses</label>
                        <input type="text" x-model="form.uses" placeholder="e.g. Cleaning"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Supplier</label>
                    <select x-model="form.supplierId"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                ...
                <div>
                    ...
                    <textarea x-model="form.description" rows="3" placeholder="Description..."
...
function materialData() {
    return {
        ...
        isEditing: false,
        saving: false,
        form: { id: null, materialName: '', categoryId: '', subcategoryId: '', price: '', totalQuantity: '', assignedQuantity: '', description: '', status: '', uses: '', supplierId: '' },
        errors: {},
        deleteId: null,
        ...
        async init() {
            await Promise.all([this.fetchCategories(), this.fetchSuppliers()]);
            this.fetchItems();
        },
        ...
        async fetchItems() {
            try {
                ...
                let res = await fetch('/api/v1/material?' + params.toString(), {
        ...
        async fetchCategories() {
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/category?per_page=100', {
                    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }
                });
                let data = await res.json();
        ...
        async fetchSubcategories() {
            if (!this.form.categoryId) { this.subcategories = []; return; }
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/subcategory?categoryId=' + this.form.categoryId + '&per_page=100', {
                    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }
                });
                let data = await res.json();
                this.subcategories = data.data?.data || data.data || [];
            } catch (e) { console.error('Subcategories fetch error:', e); }
        },

        async fetchSuppliers() {
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/supplier?per_page=100', {
                    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }
                });
                let data = await res.json();
                this.suppliers = data.data?.data || data.data || [];
            } catch (e) { console.error('Suppliers fetch error:', e); }
        },

        openAddModal() {
            this.isEditing = false;
            this.form = { id: null, materialName: '', categoryId: '', subcategoryId: '', price: '', totalQuantity: '', assignedQuantity: '', description: '', status: '', uses: '', supplierId: '' };
            this.errors = {};
            this.subcategories = [];
            this.modalOpen = true;
        },
        ...
        openEditModal(item) {
            ...
            this.form = {
                ...
                price: item.price || '',
                totalQuantity: item.totalQuantity || item.total_quantity || '',
                assignedQuantity: item.assignedQuantity || item.assigned_quantity || '',
            ...
            this.errors = {};
            if (this.form.categoryId) this.fetchSubcategories();
            this.modalOpen = true;
        },

        async saveItem() {
            this.errors = {};
            if (!this.form.materialName) { this.errors.materialName = 'Material name is required'; return; }
            this.saving = true;
            try {
                let token = localStorage.getItem('S_S_Token');
        ...
        async deleteItem() {
            ...
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/material/' + this.deleteId, {
                    method: 'DELETE',
