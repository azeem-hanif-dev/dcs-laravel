...
function materialData(){return{
    ...
    async fetchItems(){this.loading=true;try{var d=await Alpine.store('api').get('/api/v1/material',{page:this.currentPage,per_page:this.perPage,search:this.search||undefined,categoryId:this.filterCategoryId||undefined,status:this.filterStatus||undefined});if(d&&d.status){this.items=d.data?.data||d.data||[];this.total=d.data?.total||d.total||this.items.length;this.totalPages=d.data?.last_page||d.last_page||Math.ceil(this.total/this.perPage)||1}else{this.items=[];this.total=0;this.totalPages=1}}catch(e){console.error(e);this.items=[];Alpine.store('toast').error('Failed to load materials')}finally{this.loading=false}},
    async fetchCategories(){try{var d=await Alpine.store('api').get('/api/v1/category',{per_page:200});this.categories=d.data?.data||d.data||[]}catch(e){console.error(e)}},
    async fetchSubcategories(){if(!this.form.categoryId){this.subcategories=[];return};try{var d=await Alpine.store('api').get('/api/v1/subcategory',{categoryId:this.form.categoryId,per_page:200});this.subcategories=d.data?.data||d.data||[]}catch(e){console.error(e)}},
    async fetchSuppliers(){try{var d=await Alpine.store('api').get('/api/v1/supplier',{per_page:200});this.suppliers=d.data?.data||d.data||[]}catch(e){console.error(e)}},
    openAddModal(){this.isEditing=false;this.form={id:null,materialName:'',categoryId:'',subcategoryId:'',price:'',totalQuantity:'',assignedQuantity:'',status:'',uses:'',supplierId:'',description:''};this.errors={};this.subcategories=[];this.fetchCategories();this.fetchSuppliers();this.modalOpen=true},
    ...
    async saveItem(){this.errors={};if(!this.form.materialName){this.errors.materialName='Material name is required';return};this.saving=true;try{if(this.isEditing){await Alpine.store('api').put('/api/v1/material/'+this.form.id,this.form)}else{await Alpine.store('api').post('/api/v1/material',this.form)};this.closeModal();Alpine.store('toast').success(this.isEditing?'Material updated':'Material created');this.fetchItems()}catch(e){if(e.errors)this.errors=e.errors;else Alpine.store('toast').error(e.message||'Save failed')}this.saving=false},
    confirmDelete(item){this.deleteId=item.id;this.deleteModalOpen=true},
    async deleteItem(){this.saving=true;try{await Alpine.store('api').del('/api/v1/material/'+this.deleteId);this.deleteModalOpen=false;Alpine.store('toast').success('Material deleted');this.fetchItems()}catch(e){Alpine.store('toast').error(e.message||'Delete failed')}this.saving=false},
