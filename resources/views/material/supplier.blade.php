...
@section('title', 'Supplier Management - Digital Clean Solution')

@section('page-content')
<div x-data="supplierData()" x-init="fetchItems()" class="max-w-7xl mx-auto px-4">

    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:w-72">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" x-model="search" @input.debounce.300ms="fetchItems()" placeholder="Search suppliers..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Address</th>
                        <th class="px-4 py-4 text-center text-xs font-semibold text-white uppercase tracking-wider">Action</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="items.length === 0">
                        <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">No suppliers found.</td></tr>
                    </template>
                    <template x-for="(item, index) in items" :key="item.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.contactNumber || item.phone || ''"></td>
                            <td class="px-4 py-4 text-sm text-gray-600 max-w-[200px] truncate" x-text="item.address || ''"></td>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="saveItem()" class="space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.name" placeholder="Supplier name"
                ...
                <div>
                    ...
                    <input type="email" x-model="form.email" placeholder="supplier@example.com"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
...
function supplierData() {
    return {
        ...
        isEditing: false,
        saving: false,
        form: { id: null, name: '', email: '', contactPerson: '', contactNumber: '', address: '' },
        errors: {},
        deleteId: null,
        ...
        async fetchItems() {
            try {
                ...
                let res = await fetch('/api/v1/supplier?' + params.toString(), {
        ...
        openAddModal() {
            ...
            this.form = { id: null, name: '', email: '', contactPerson: '', contactNumber: '', address: '' };
            this.errors = {};
            this.modalOpen = true;
        },
        ...
        async saveItem() {
            this.errors = {};
            if (!this.form.name) { this.errors.name = 'Supplier name is required'; return; }
            this.saving = true;
            try {
                let token = localStorage.getItem('S_S_Token');
        ...
        async deleteItem() {
            ...
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/supplier/' + this.deleteId, {
                    method: 'DELETE',
