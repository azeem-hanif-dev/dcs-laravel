    ...
    <div class="bg-white rounded-xl shadow-sm p-3 mb-4 border border-gray-100">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1"><svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <input type="text" x-model="search" @input.debounce.300ms="currentPage=1;fetchItems()" placeholder="Search suppliers..." class="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none text-sm"></div>
            <button @click="search=''; currentPage=1; fetchItems()" class="text-sm text-gray-500 hover:text-gray-700 px-3 py-2 rounded-lg hover:bg-gray-100 whitespace-nowrap">Clear</button>
        </div>
    </div>
            ...
            <table class="table-card-sm min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Address</th>
                        <th class="px-3 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-28">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="loading">
                        <tr><td colspan="6" class="px-6 py-16 text-center"><div class="flex flex-col items-center gap-2"><svg class="spinner w-8 h-8 text-primary" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg><span class="text-sm text-gray-400">Loading...</span></div></td></tr>
                    </template>
                    <template x-if="!loading && items.length === 0">
                        <tr><td colspan="6" class="px-6 py-16 text-center"><svg class="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg><p class="text-gray-500 font-medium">No suppliers found</p><p class="text-sm text-gray-400 mt-1">Add your first supplier</p></td></tr>
                    </template>
                    <template x-for="(item, index) in items" :key="item.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-3 py-3 text-sm text-gray-600 whitespace-nowrap hidden md:table-cell" data-label="Phone" x-text="item.contactNumber||item.phone||''"></td>
                            <td class="px-3 py-3 text-sm text-gray-600 max-w-[180px] truncate hidden lg:table-cell" data-label="Address" x-text="item.address||''"></td>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 max-h-[90vh] overflow-y-auto" @click.outside="closeModal()">
            ...
            <form @submit.prevent="saveItem()" class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.name" placeholder="Supplier name" class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none">
                    <p x-show="errors.name" class="text-red-500 text-xs mt-1" x-text="errors.name"></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Email <span class="text-red-500">*</span></label>
                    <input type="email" x-model="form.email" placeholder="supplier@example.com" class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none">
...
function supplierData(){return{
    items:[],search:'',currentPage:1,perPage:10,total:0,totalPages:1,loading:false,modalOpen:false,deleteModalOpen:false,isEditing:false,saving:false,form:{id:null,name:'',email:'',contactPerson:'',contactNumber:'',address:''},errors:{},deleteId:null,
    ...
    async fetchItems(){this.loading=true;try{let d=await $store.api.get('/api/v1/supplier',{page:this.currentPage,per_page:this.perPage,search:this.search||undefined});if(d.status){this.items=d.data?.data||d.data||[];this.total=d.data?.total||d.total||this.items.length;this.totalPages=d.data?.last_page||d.last_page||Math.ceil(this.total/this.perPage)||1}}catch(e){$store.toast.error('Failed to load suppliers')}this.loading=false},
    ...
    openEditModal(item){this.isEditing=true;this.form={id:item.id,name:item.name||'',email:item.email||'',contactPerson:item.contactPerson||item.phone||'',contactNumber:item.contactNumber||item.phone||'',address:item.address||''};this.errors={};this.modalOpen=true},
    closeModal(){this.modalOpen=false;this.form={id:null,name:'',email:'',contactPerson:'',contactNumber:'',address:''};this.errors={}},
    async saveItem(){this.errors={};if(!this.form.name){this.errors.name='Name is required';return};if(!this.form.email){this.errors.email='Email is required';return};this.saving=true;try{if(this.isEditing){await $store.api.put('/api/v1/supplier/'+this.form.id,this.form)}else{await $store.api.post('/api/v1/supplier',this.form)};this.closeModal();$store.toast.success(this.isEditing?'Supplier updated':'Supplier created');this.fetchItems()}catch(e){if(e.errors)this.errors=e.errors;else $store.toast.error(e.message||'Save failed')}this.saving=false},
    confirmDelete(item){this.deleteId=item.id;this.deleteModalOpen=true},
    async deleteItem(){this.saving=true;try{await $store.api.del('/api/v1/supplier/'+this.deleteId);this.deleteModalOpen=false;$store.toast.success('Supplier deleted');this.fetchItems()}catch(e){$store.toast.error(e.message||'Delete failed')}this.saving=false},
