...
@section('title', 'Material Category Management - Digital Clean Solution')

@section('page-content')
<div x-data="categoryData()" x-init="fetchItems()" class="max-w-4xl mx-auto px-4">

    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:w-72">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" x-model="search" @input.debounce.300ms="fetchItems()" placeholder="Search categories..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="items.length === 0">
                        <tr><td colspan="3" class="px-6 py-12 text-center text-gray-500">No categories found.</td></tr>
                    </template>
    ...
    return {
        ...
        async fetchItems() {
            try {
                ...
                let res = await fetch('/api/v1/category?' + params.toString(), {
        ...
        async saveItem() {
            this.errors = {};
            if (!this.form.name) { this.errors.name = 'Category name is required'; return; }
            this.saving = true;
            try {
                let token = localStorage.getItem('S_S_Token');
        ...
        async deleteItem() {
            ...
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/category/' + this.deleteId, {
                    method: 'DELETE',
