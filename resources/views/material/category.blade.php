{{-- resources/views/material/category.blade.php --}}
@extends('layouts.admin')

@section('title', 'Categories - Distributor Portal')

@section('page-content')
<div x-data="categoryData()" x-init="fetchItems()" class="max-w-7xl mx-auto px-2 sm:px-4">

    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Categories</h1>
            <p class="text-sm text-gray-500 mt-0.5">Manage material categories</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-2.5 mb-3 border border-gray-100">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300ms="currentPage=1;fetchItems()" placeholder="Search categories..."
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <template x-if="!loading && items.length === 0">
                        <tr><td colspan="3" class="px-6 py-16 text-center">
                            ...
                            <p class="text-gray-500 font-medium">No categories found</p><p class="text-sm text-gray-400 mt-1">Click "Add Category" to create one</p>
                        </td></tr>
                    </template>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity.duration.200>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10" @click.outside="closeModal()">
            <div class="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-white rounded-t-2xl z-10">
                <h3 class="text-lg font-semibold text-gray-800" x-text="isEditing ? 'Edit Category' : 'Add Category'"></h3>
            ...
            <form @submit.prevent="saveItem()" class="p-6 space-y-4">
                <div>
                    ...
                    <select x-model="form.name" class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none">
                        <option value="">Select Category</option>
                        <option value="Chemical">Chemical</option><option value="Hard Material">Hard Material</option><option value="Textile">Textile</option><option value="Paper">Paper</option><option value="Machine">Machine</option>
                    </select>
...
function categoryData(){return{
    ...
    async fetchItems(){this.loading=true;try{var d=await Alpine.store('api').get('/api/v1/category',{page:this.currentPage,per_page:this.perPage,search:this.search||undefined});if(d&&d.status){this.items=d.data?.data||d.data||[];this.total=d.data?.total||d.total||this.items.length;this.totalPages=d.data?.last_page||d.last_page||Math.ceil(this.total/this.perPage)||1}else{this.items=[];this.total=0;this.totalPages=1}}catch(e){console.error(e);this.items=[];Alpine.store('toast').error('Failed to load categories')}finally{this.loading=false}},
    ...
    async saveItem(){this.errors={};if(!this.form.name){this.errors.name='Category name is required';return}this.saving=true;try{if(this.isEditing){await Alpine.store('api').put('/api/v1/category/'+this.form.id,{name:this.form.name})}else{await Alpine.store('api').post('/api/v1/category',{name:this.form.name})}this.closeModal();Alpine.store('toast').success(this.isEditing?'Category updated':'Category created');this.fetchItems()}catch(e){if(e.errors)this.errors=e.errors;else Alpine.store('toast').error(e.message||'Save failed')}this.saving=false},
    confirmDelete(item){this.deleteId=item.id;this.deleteModalOpen=true},
    async deleteItem(){this.saving=true;try{await Alpine.store('api').del('/api/v1/category/'+this.deleteId);this.deleteModalOpen=false;Alpine.store('toast').success('Category deleted');this.fetchItems()}catch(e){Alpine.store('toast').error(e.message||'Delete failed')}this.saving=false},
