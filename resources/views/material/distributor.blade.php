...
@section('title', 'Distributor Management - Digital Clean Solution')

@section('page-content')
<div x-data="distributorData()" x-init="init()" class="max-w-7xl mx-auto px-4">

    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full sm:w-72">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" x-model="search" @input.debounce.300ms="fetchItems()" placeholder="Search distributors..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Logo</th>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Supplier</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <template x-if="items.length === 0">
                        <tr><td colspan="7" class="px-6 py-12 text-center text-gray-500">No distributors found.</td></tr>
                    </template>
                    <template x-for="(item, index) in items" :key="item.id">
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                <img :src="item.logo ? '/storage/' + item.logo : '/common/distributor-logo.svg'" class="w-10 h-10 rounded-full object-cover border border-gray-200" alt="Logo">
                            </td>
                            ...
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.supplier?.name || ''"></td>
                            <td class="px-4 py-4 text-sm text-gray-700 whitespace-nowrap" x-text="item.contactNumber || item.contact_number || ''"></td>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
            ...
            <form @submit.prevent="saveItem()" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" x-model="form.name" placeholder="Distributor name"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    <p x-show="errors.name" class="text-red-500 text-xs mt-1" x-text="errors.name"></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                    <input type="email" x-model="form.email" placeholder="distributor@example.com"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="form.contactNumber" placeholder="Phone number"
                            class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div x-show="logoPreview" class="mt-2">
                        <img :src="logoPreview" class="w-16 h-16 rounded-full object-cover border border-gray-200" alt="Logo preview">
                    </div>
                </div>
...
function distributorData() {
    return {
        ...
        logoPreview: null,
        logoFile: null,
        form: { id: null, name: '', email: '', contactPerson: '', contactNumber: '', address: '', supplierId: '' },
        errors: {},
        deleteId: null,
        ...
        async fetchSuppliers() {
            try {
                ...
                let res = await fetch('/api/v1/supplier?per_page=200', {
        ...
        async fetchItems() {
            try {
                ...
                let res = await fetch('/api/v1/distributor?' + params.toString(), {
                    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }
                });
                let data = await res.json();
        ...
        openAddModal() {
            ...
            this.form = { id: null, name: '', email: '', contactPerson: '', contactNumber: '', address: '', supplierId: '' };
            this.errors = {};
        ...
        openEditModal(item) {
            ...
            this.logoPreview = item.logo ? '/storage/' + item.logo : null;
            this.logoFile = null;
            this.errors = {};
            this.modalOpen = true;
        },
        ...
        async saveItem() {
            this.errors = {};
            if (!this.form.name) { this.errors.name = 'Distributor name is required'; return; }
            if (!this.form.supplierId) { this.errors.supplierId = 'Supplier is required'; return; }
            this.saving = true;
            try {
                let token = localStorage.getItem('S_S_Token');
                let url = '/api/v1/distributor';
                let method = 'POST';

                let formData = new FormData();
                formData.append('name', this.form.name);
                formData.append('email', this.form.email);
                if (this.form.contactPerson) formData.append('contactPerson', this.form.contactPerson);
                if (this.form.contactNumber) formData.append('contactNumber', this.form.contactNumber);
        ...
        async deleteItem() {
            ...
            try {
                let token = localStorage.getItem('S_S_Token');
                let res = await fetch('/api/v1/distributor/' + this.deleteId, {
                    method: 'DELETE',
                    headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' }
                });
